...
import org.fourprimes.db.mongodb.routing.Insert
import org.fourprimes.db.mongodb.routing.Select
import com.mongodb.DBObject
import akka.dispatch.OnComplete

// we don't implement our route structure directly in the service actor because
// we want to be able to test it independently, without having to spin up an actor
class MongoDBServiceActor extends Actor with MongoDBService {
...
trait MongoDBService extends HttpService {
  implicit val timeout:akka.util.Timeout = 1 second          
  
  val mongoDBRouter = actorRefFactory.actorOf(
  ...
  val routes = {
    get {
      path("") {
        ...
        path("props" / PathElement) { key =>
          complete {
            (mongoDBRouter ? Select(key)).mapTo[String]
          }
        } ~
        path("stream1") {
      ...
      (post | parameter('method ! "post")) {
        path("props") {
          formFields('key.as[String], 'value.as[String]) { (key, value) =>
            complete {
              mongoDBRouter ! Insert(key, value)
              key + "->" + value
            }
          }
        } ~
